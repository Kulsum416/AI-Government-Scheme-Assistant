# AI-Powered Government Scheme Assistant

## Project Overview
This project is an AI-powered chatbot designed to help users easily understand Indian government schemes. The assistant provides structured, clear, and uniform information so that citizens can quickly know whether a scheme is useful for them.

## Problem Statement
Many citizens are unable to benefit from government schemes due to lack of awareness and complicated information scattered across multiple sources. There is a need for a simple AI-based system that presents scheme information in an understandable format.

## Solution Description
The Government Scheme Assistant uses AI to retrieve information and present it in a fixed structure:
- Scheme Name
- Eligibility
- Benefits
- How to Apply
- Notes (if any)

## Key Features
- User-friendly chatbot interface
- Structured and uniform scheme details
- AI-based summarization of information
- Supports awareness and informed decision-making

## Technologies Used
- Python
- Streamlit
- Wikipedia (for online information)
- AI-based text processing

## SDG Alignment
- SDG 10: Reduced Inequalities
- SDG 16: Peace, Justice & Strong Institutions

## Team Members
- M Umme Kulsum
    
##  Project Screenshots

### 1️. Home Page
![Home Page](screenshots/home_page.png)

### 2️. Scheme Search
![Scheme Search](screenshots/scheme_search.png)

### 3️. Result Display
![Result Display](screenshots/result_display.png)

### 4️. Online Fetch (When Scheme Not Found Locally)
![Online Fetch](screenshots/online_fetch.png)

### 5️. Online Fetch Result (Structured Output)
![Online Fetch Result](screenshots/online_fetch_result.png)
